...
import SidePosterComponent from "./side-poster";
...
export default function SpreadsheetSidePosters({ left, right, children }: Props) {
  ...
  return (
    <div className="relative" style={{ minHeight }}>
      {mounted && (
        <>
          <div ref={leftRef} className="absolute top-0 -left-[380px] hidden 3xl:flex flex-col gap-4">
            {left.map((poster, i) => (
          ...
          <div ref={rightRef} className="absolute top-0 -right-[380px] hidden 3xl:flex flex-col gap-4">
            {right.map((poster, i) => (
              <div key={i} className="w-[300px] h-[800px]">
                <SidePosterComponent poster={poster} />
              </div>
